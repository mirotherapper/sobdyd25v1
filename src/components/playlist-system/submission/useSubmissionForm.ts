    ...
    */
    ...
    try {
      ...
      const dataToSend = {
        url,
        submissionType:
          queUpData.find(q => q.id === selectedQueUp)?.name || 'FREE CHANCE',
        submission_message: submissionMessage, // Use correct field name
        submissionMessage, // Keep both for compatibility
        startTime,
        platform,
        metadata,
        submittedBy: userId,
