...
export async function POST(request: Request) {
  try {
    ...
    const livePlaylistForLockCheck = await db
      .collection('playlists')
      .findOne({ is_show_archive: { $ne: true } });
    ...
    if (livePlaylistForLockCheck?.is_locked) {
      ...
      if (source.droppableId === 'queue' && source.index < 10) {
        return NextResponse.json(
          { message: 'Queue is locked. Cannot move a trax from the top 10.' },
          { status: 403 }
        );
      }
      ...
      if (destination.droppableId === 'queue' && destination.index < 10) {
        return NextResponse.json(
          { message: 'Queue is locked. Cannot move a trax into the top 10.' },
          { status: 403 }
        );
      }
    }
    ...
    if (
      source.droppableId === 'submissions' &&
      ...
      const livePlaylist = await db
        .collection('playlists')
        .findOne({ is_show_archive: { $ne: true } });

      if (!livePlaylist) {
        return NextResponse.json(
          { message: 'Live playlist not found' },
          { status: 404 }
        );
      }
      ...
      const newPlaylistItem: Omit<PlaylistItemData, 'position'> = {
        ...
        status: 'queued',
        queUp: submission.submissionType,
      ...
      return NextResponse.json(
        { message: 'Trax moved successfully' },
        { status: 200 }
      );
    }
    ...
    if (source.droppableId === 'queue' && destination.droppableId === 'queue') {
      ...
      const queuedItems = allItems
        ...
        .sort(
          (a: PlaylistItemData, b: PlaylistItemData) => a.position - b.position
        );
      const otherItems = allItems.filter(
        (item: PlaylistItemData) => item.status !== 'queued'
      );

      const movedItemIndex = queuedItems.findIndex(
        (item: PlaylistItemData) => item._id === draggableId
      );
      if (movedItemIndex === -1) {
        return NextResponse.json(
          {
            message:
              'Trax to move not found in queue. State may be out of sync.',
          },
          { status: 404 }
        );
      }
      ...
      const newFullItemsList = [...otherItems, ...queuedItems].map(
        (item: PlaylistItemData, index: number) => ({
          ...item,
          position: index,
        })
      );
    ...
    if (
      ...
      const movedItemIndex = allItems.findIndex(
        (item: PlaylistItemData) => item._id === draggableId
      );
      if (movedItemIndex === -1) {
        return NextResponse.json(
          { message: 'Track to move not found. State may be out of sync.' },
          { status: 404 }
        );
      }
      ...
      const playedItems = allItems
        .filter((item: PlaylistItemData) => item.status === 'played')
        .sort(
          (a: PlaylistItemData, b: PlaylistItemData) => a.position - b.position
        );
      const otherItems = allItems.filter(
        (item: PlaylistItemData) =>
          item.status !== 'queued' && item.status !== 'played'
      ); // e.g., now_playing
      ...
      const newFullItemsList = [
        ...otherItems,
        ...queuedItems,
        ...playedItems,
      ].map((item: PlaylistItemData, index: number) => ({
        ...item,
        position: index,
      }));
      ...
      return NextResponse.json(
        { message: 'Track moved to played successfully' },
        { status: 200 }
      );
    }
    return NextResponse.json(
      { message: 'This move operation is not supported' },
      { status: 400 }
    );
  } catch (error) {
    ...
    return NextResponse.json(
      { message: 'Error managing track', error: errorMessage },
      { status: 500 }
    );
  }
}
